...
export const TARGET_FORMATS_BY_CATEGORY: Record<string, string[]> = {
  image: ['PNG', 'JPG', 'WEBP', 'GIF', 'BMP', 'ICO', 'TIFF', 'AVIF', 'PDF', 'DOCX', 'XLSX', 'CSV', 'TXT', 'HTML'],
...
export const CATEGORY_LABELS: Record<string, Record<string, string>> = {
  ru: {
    ...
    audio: '🎵 Аудио форматы',
    video: '🎬 Видео форматы',
    document: '📄 Документы и таблицы (Beta)',
    archive: '📦 Архивы',
    '3d': '🧊 3D Модели',
  },
  en: {
    ...
    audio: '🎵 Audio formats',
    video: '🎬 Video formats',
    document: '📄 Documents & Spreadsheets (Beta)',
    archive: '📦 Archives',
    '3d': '🧊 3D Models',
  },
  zh: {
  ...
  es: {
    ...
    document: '📄 Documentos y tablas (Beta)',
    archive: '📦 Archivos',
    '3d': '🧊 Modelos 3D',
  },
  de: {
    ...
    audio: '🎵 Audioformate',
    video: '🎬 Videoformate',
    document: '📄 Dokumente & Tabellen (Beta)',
    archive: '📦 Archive',
    '3d': '🧊 3D-Modelle',
  },
};
...
export function getAvailableTargets(
  ...
  if (category !== 'all' && TARGET_FORMATS_BY_CATEGORY[category]) {
    ...
    // For 'all', filter out non-target document formats like EPUB, XLS
    const nonExportTargets = ['EPUB', 'XLS'];
